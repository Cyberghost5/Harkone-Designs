...
// Fetch all published projects in descending order (latest first)
$stmt = $pdo->prepare("SELECT * FROM `projects` WHERE `status` = 'published' ORDER BY `is_featured` DESC, `id` DESC");
$stmt->execute();
$dbProjects = $stmt->fetchAll();

// Construct projects list for Alpine JS
$projectsList = [];
$categoryCounts = [
    'all' => count($dbProjects),
    'saas' => 0,
    'landing' => 0,
    'agency' => 0,
    'ecommerce' => 0
...
foreach ($dbProjects as $p) {
    ...
    $tagsArray = array_map('trim', explode(',', $p['tags']));
    ...
    $projectsList[] = [
        ...
        'img' => $p['image_url'],
        'url' => $p['project_url'],
        'slug' => "./case-study.php?slug=" . $p['slug']
    ];
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <link
    ...
    rel="stylesheet" />
  <script>tailwind.config = { darkMode: 'class', theme: { extend: { fontFamily: { display: ['PT Sans', 'sans-serif'], body: ['DM Sans', 'sans-serif'] }, colors: { accent: '#0b94baff', 'accent-light': '#2dd902ff' } } } }</script>
  <style>
    *,
    *::before,
    *::after {
  ...
  <header class="fixed top-0 left-0 right-0 z-50 transition-all duration-300"
    ...
    <div x-show="mobileMenu" x-cloak x-transition:enter="transition ease-out duration-200"
      ...
      <ul class="flex flex-col px-6 py-4 gap-4 text-sm font-medium">
        ...
        <li><a href="projects" class="block text-accent font-medium">Work</a></li>
  ...
  <main>
    <section class="pt-36 pb-12 relative overflow-hidden">
      ...
      <div class="max-w-6xl mx-auto px-6 relative z-10">
        ...
        <h1 class="reveal font-display font-bold text-5xl md:text-6xl text-zinc-900 dark:text-white leading-tight mb-4">
          All Projects</h1>
        <p class="reveal text-lg text-zinc-500 dark:text-zinc-400 max-w-xl leading-relaxed mb-8">A complete look at my
          work across UI/UX design, frontend development and landing pages.</p>
        <div class="reveal flex flex-wrap gap-2">
          <template x-for="f in filterOptions" :key="f.value">
            <button @click="filter = f.value"
              ...
              class="text-sm px-4 py-1.5 rounded-full border transition-colors" x-text="f.label">
            </button>
          </template>
        </div>
      </div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          <template x-for="p in allProjects" :key="p.id">
            <div x-show="filter === 'all' || p.category === filter"
              ...
              <article
                class="project-card group h-full rounded-2xl overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 hover:border-accent transition-colors">
                <a :href="p.slug" class="block photo-frame w-full h-52">
                ...
                <div class="p-6">
                  <div class="flex flex-wrap gap-2 mb-3">
                    <template x-for="tag in p.tags" :key="tag">
                      <span class="text-xs px-2.5 py-1 rounded-full"
                        :class="tag === p.categoryLabel ? 'dark:bg-zinc-800 text-accent border dark:border-zinc-700' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400'"
                        x-text="tag"></span>
                    </template>
                  </div>
                  <a :href="p.slug">
                  ...
                  <div class="flex items-center justify-between">
                    <a :href="p.slug"
                      class="inline-flex items-center gap-1.5 text-sm font-medium text-zinc-900 dark:text-white nav-link">View
                      project →</a>
      ...
      <div class="max-w-6xl mx-auto px-6">
        <div class="bg-zinc-900 dark:bg-zinc-800 rounded-3xl p-10 md:p-14 text-center relative overflow-hidden">
          ...
          <div class="relative z-10">
            ...
            <a href="./#contact"
              class="inline-flex items-center gap-2 btn-primary bg-accent text-white font-medium px-8 py-3.5 rounded-full hover:bg-accent-light transition-colors">Start
              a project &rarr;</a>
          </div>
        </div>
      </div>
  ...
  <script>
    function projects() {
      return {
        ...
        filterOptions: [
          { value: 'all', label: 'All (<?php echo $categoryCounts['all']; ?>)' },
          { value: 'saas', label: 'SaaS (<?php echo $categoryCounts['saas']; ?>)' },
          { value: 'landing', label: 'Landing page (<?php echo $categoryCounts['landing']; ?>)' },
          { value: 'agency', label: 'Agency (<?php echo $categoryCounts['agency']; ?>)' },
          { value: 'ecommerce', label: 'E-commerce (<?php echo $categoryCounts['ecommerce']; ?>)' },
        ],

        allProjects: <?php echo json_encode($projectsList); ?>,
